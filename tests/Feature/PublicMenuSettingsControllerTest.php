...
class PublicMenuSettingsControllerTest extends TestCase
{
    ...
    public function test_can_update_public_menu_settings(): void
    {
        ...
        $response
            ->assertStatus(200)
            ->assertJsonPath('data.public_card_url', 'le-resto')
            ->assertJsonPath('data.only_sufficient_stock', true)
            ->assertJsonPath('data.with_pictures', false);

        $company->refresh();

        $this->assertSame('le-resto', $company->public_card_url);
        $this->assertFalse($company->show_out_of_stock_menus);
        $this->assertFalse($company->show_menu_images);
    }

    public function test_public_card_url_must_be_unique(): void
    {
        $company = Company::factory()->create(['public_card_url' => 'unique-url']);
        $otherCompany = Company::factory()->create();
        $user = User::factory()->create(['company_id' => $otherCompany->id]);

        $this->actingAs($user)
            ->putJson('/api/public-menus', [
                'public_card_url' => 'unique-url',
            ])
            ->assertStatus(422)
            ->assertJsonPath('errors.public_card_url.0', 'ALREADY_TAKEN, désolé mais cette URL est déjà prise');
    }

    public function test_public_card_url_cannot_be_empty(): void
    {
        $company = Company::factory()->create();
        $user = User::factory()->create(['company_id' => $company->id]);
